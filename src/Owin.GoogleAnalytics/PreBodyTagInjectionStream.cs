		...
		public override void Flush()
		{
			if (!string.IsNullOrEmpty(UnwrittenCharactersFromPreviousCall))
			{
				...
				else
				{
					...
					Logger.WriteWarning("Unable to locate '</body>' with content encoding '{0}' for request '{1}'. The response may be compressed or the markup may actually be missing a '</body>' tag.", ContentEncoding.EncodingName, CurrentRequestRawUrl);
				}
